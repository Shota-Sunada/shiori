          ...
          {rollCalls.map((rollCall) => (
            <tr key={rollCall.id} className={`${!rollCall.is_active ? 'bg-gray-200' : ''}`}>
              <td className="py-2 px-4 border-b text-center">
                {rollCall.is_active ? (
                  <span className="bg-green-500 text-white py-1 px-2 rounded">発動中</span>
                ) : (
                  <span className="bg-gray-500 text-white py-1 px-2 rounded">終了</span>
                )}
              </td>
              <td className="py-2 px-4 border-b text-center">{new Date(rollCall.created_at).toLocaleString()}</td>
              <td className="py-2 px-4 border-b text-center">{rollCall.checked_in_students} / {rollCall.total_students}</td>
              <td className="py-2 px-4 border-b text-center">{rollCall.teacher_id}</td>
              <td className="py-2 px-4 border-b text-center">
                <Link to={`/teacher/call?id=${rollCall.id}`} className="text-blue-500 hover:underline">
                  詳細を見る
                </Link>
              </td>
            </tr>
