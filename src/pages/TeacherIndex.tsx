...
const TeacherIndex = () => {
  ...
  useEffect(() => {
    if (!loading && !user) {
      navigate('/login');
    } else if (user && user.userId && token) {
      const fetchTeacherName = async () => {
        try {
          ...
          console.error("Failed to fetch teacher's name:", error);
          setTeacherData(null);
        }
      };
      fetchTeacherName();
  ...
  return (
    <div className="flex flex-col items-center justify-center m-[10px]">
      <div className="m-2 flex flex-col items-center justify-center">
        ...
        <p className="text-xl">{teacherData ? `${teacherData.surname} ${teacherData.forename} 先生` : "管理者"}{"としてログイン中"}</p>
      </div>
